// 2>nul||@goto :batch
/* THIS FILE IS DERIVED FROM:  `npocmaka/batch.scripts` under the MIT license: https://github.com/npocmaka/batch.scripts/blob/master/hybrids/.net/c/screenCapture.bat
:batch
@echo off
setlocal
...
if not exist "%TEMP%\%~n0.exe" (
   call %csc% /nologo /r:"Microsoft.VisualBasic.dll" /out:"%TEMP%\%~n0.exe" "%~0" || (
      exit /b %errorlevel% 
   )
)
"%TEMP%\%~n0.exe" %*
